...
const PortRequestManagement = () => {
  ...
  const handleSubmit = (e) => {
    ...
    CustomerService.submitPortRequest(newPortRequest)
      .then(() => {
        alert('Port Request added successfully');
        setNewPortRequest({
          username: '',
          requestDate: ''
        });
      ...
      .catch(error => {
        console.error('Error adding port request:', error);
        alert('An error occurred while adding port request');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const fetchPortRequest = async (requestId) => {
    try {
      ...
      setSelectedPortRequest(portRequest); // Set the selected port request
    } catch (error) {
